...
void Device::compile_command_queue_programs() {
    ...
    if (this->is_mmio_capable()) {
        for (const chip_id_t &device_id : tt::Cluster::instance().get_devices_controlled_by_mmio_device(this->id())) {
            ...
            for (uint8_t cq_id = 0; cq_id < num_hw_cqs; cq_id++) {
                ...
                if (device_id != this->id()) {
                    ...
                    //     prefetcher_location, EthRouterMode::BI_DIR_TUNNELING, device_id);
                    ...
                    //     {"PRODUCER_NOC_X", std::to_string(prefetcher_physical_core.x)},
                    //     {"PRODUCER_NOC_Y", std::to_string(prefetcher_physical_core.y)},
                    // };
        ...
        // tt_cxy_pair dispatch_location = dispatch_core_manager::get(num_hw_cqs).dispatcher_core(this->id(), channel, cq_id);
